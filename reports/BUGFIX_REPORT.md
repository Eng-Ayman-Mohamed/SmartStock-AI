...
- **Follow-up (multi-page):** Extended to rasterize **every page** (capped at `MAX_PDF_PAGES = 5` to bound tokens) and send all pages in one vision request, so multi-page invoices keep all line items. Verified with a generated 2-page invoice (items split 3/2 across pages) → all **5 line items** extracted; single-page PDF and PNG paths unchanged (still 5/5). Commit `fix(ingestion): rasterize all PDF pages, not just the first`.
- **Notes:** The running dev container had `poppler-utils`/`pdf2image` installed ad-hoc for verification; a `docker compose build backend` bakes them in from the committed Dockerfile/requirements.
