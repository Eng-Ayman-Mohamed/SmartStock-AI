...
class PurchasingService:
    ...
    def reject_po(self, po_id: int, user):
        ...
        if po.status not in ("draft", "pending_approval"):
            raise ValidationError(
                "Only draft or pending approval orders can be rejected."
            )
    ...
    def mark_email_sent(self, po_id: int, message_id: str | None = None):
        ...
        if po.status not in ("approved", "sent"):
            raise ValidationError(
                "Only approved or sent orders can be marked as email sent."
            )
        update_data: dict = {
            "status": "email_sent",
            "sent_at": timezone.now(),
    ...
    def mark_waiting_confirmation(self, po_id: int):
        ...
        if po.status != "email_sent":
            raise ValidationError(
                "Only email_sent orders can be moved to waiting confirmation."
            )
