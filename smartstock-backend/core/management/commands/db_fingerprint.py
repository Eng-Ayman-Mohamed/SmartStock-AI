...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            ...
            # ---- 2. Migration list ----
            cursor.execute(
                "SELECT app, name FROM django_migrations ORDER BY app, applied"
            )
            ...
            ts_models = [
                m for m in all_models if hasattr(m, "_meta")
            ]
            ...
            hash_input_parts.extend(
                f"{app}:{name}" for app, name in migrations
            )
            hash_input_parts.extend(
                f"{table}:{count}" for table, count in table_rows.items()
            )
            hash_input_parts.extend(
                f"{table}:{ts}" for table, ts in latest_entries.items()
            )
