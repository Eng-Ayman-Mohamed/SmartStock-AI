...
from ai.llm.schemas import NLQueryAction, NLQueryFilters, NLQueryResult, NL_QUERY_JSON_SCHEMA
...
class TestNLQueryChainEndToEnd:
    ...
    def test_query_type_end_to_end(
        ...
        assert getattr(result.filters, filter_key) == filter_val, (
            f"[{query_type}] Expected filters.{filter_key}={filter_val!r}, "
            f"got {getattr(result.filters, filter_key)!r}"
        )
