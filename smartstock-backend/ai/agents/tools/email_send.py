...
class EmailSendTool(BaseTool):
    ...
    def run(self, input: dict) -> dict:
        try:
            ...
            body = self._build_email_body(
                po_id, sku_code, product_name, po.quantity, po.total_cost,
                po.requested_by, supplier_name,
            )
            ...
            logger.info('PO email sent: PO-%s to %s (message_id=%s)', po_id, recipient_email, message_id)
            return {
                'status': 'sent',
                'po_id': po_id,
    ...
    def _build_email_body(
        self, po_id, sku_code, product_name, quantity, total_cost,
        requested_by, supplier_name,
    ) -> str:
        return (
