...
export default function AIAssistantPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'ai', text: 'Hello! I\'m your AI inventory assistant. Ask me anything about your stock, forecasts, or suppliers.' },
  ]);
  ...
  return (
    <div className="h-[calc(100vh-40px-64px)] animate-fadeIn flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-page-heading text-gray-900">AI Assistant</h1>
          <p className="text-body text-gray-600 mt-1">Ask anything about your inventory</p>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-6 min-h-0">
        <Card noPadding className="flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-[80%] ${
                  msg.role === 'user'
                    ...
                    : 'bg-gray-50 text-gray-900 rounded-lg rounded-bl-sm px-4 py-2.5'
                }`}>
            ...
            {isThinking && (
              <div className="flex gap-3">
                ...
                <div className="bg-gray-50 rounded-lg rounded-bl-sm px-4 py-3">
          ...
          <div className="px-5 py-3 border-t-[0.5px] border-gray-100">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 h-9 px-4 rounded-full border-[0.5px] border-gray-100 bg-white text-body text-gray-900 placeholder:text-gray-400 hover:border-gray-400 focus:border-brand-600 focus:outline-none focus:ring-0 transition-colors"
              ...
              <button
                ...
                className="flex items-center justify-center w-9 h-9 rounded-full bg-brand-600 text-white hover:bg-brand-800 disabled:bg-gray-100 disabled:text-gray-400 transition-colors shrink-0"
          ...
          <Card title="Current Inventory Snapshot">
            <div className="space-y-3">
              {[
                ...
                <div key={item.name} className="flex items-center gap-3 pb-2 border-b-[0.5px] border-gray-100 last:border-0 last:pb-0">
                  <div className={`flex items-center justify-center w-7 h-7 rounded-md shrink-0 ${
                    item.stock < item.threshold ? 'bg-amber-50' : 'bg-green-50'
                  }`}>
                    <item.icon className={`w-4 h-4 ${item.stock < item.threshold ? 'text-amber-600' : 'text-green-600'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-body text-gray-900 truncate">{item.name}</p>
                    <p className="text-caption text-gray-600 tabular-nums">{item.stock} units</p>
                  </div>
                  {item.stock < item.threshold && <AlertTriangle className="w-4 h-4 text-amber-600" />}
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card title="Data Sources">
            <div className="space-y-2">
              {[
                ...
                <div key={src.name} className="flex items-center gap-2 text-body text-gray-600">
